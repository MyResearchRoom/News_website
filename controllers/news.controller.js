...
exports.incrementViewCount = async (req, res) => {
  ...
  try {
    ...
    const existingView = await NewsView.findOne({
      where: { newsId: id, visitorId },
      transaction,
    });

    if (existingView) {
      return errorResponse(res, "View already counted", 200);
    }

    await NewsView.create({ newsId: id, visitorId }, { transaction });

    await News.increment("viewCount", { by: 1, where: { id }, transaction });

    await transaction.commit();

    successResponse(res, "View counted successfully");
  } catch (error) {
    await transaction.rollback();
    errorResponse(res, "Failed to update view count", 500);
  }
};
